...
export function CreateOrder() {
  ...
  const availableTokens = balances.filter((balance) => parseFloat(balance.exchangeBalance) > 0);
  ...
  if (availableTokens.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <Alert>
            <AlertDescription>请先存款代币到交易所才能创建订单</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="tokenGiveAmount">给出数量 ({getTokenSymbol(tokenGive)})</Label>
            <div className="flex gap-2">
              ...
              />
              {tokenGive && (
                <Button variant="outline" size="sm" onClick={() => setMaxAmount(tokenGive)}>
                  MAX
                </Button>
              )}
            </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="tokenGetAmount">获得数量 ({getTokenSymbol(tokenGet)})</Label>
            <Input
              id="tokenGetAmount"
              type="number"
              step="0.0001"
          ...
          <div className="p-4 bg-muted rounded-lg">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono">
                1 {getTokenSymbol(tokenGive)} = {calculateRate()} {getTokenSymbol(tokenGet)}
              </span>
            </div>
          </div>
        )}
        ...
        <Button
          ...
          disabled={orderLoading || loading || !tokenGive || !tokenGet || !tokenGiveAmount || !tokenGetAmount}
          className="w-full"
          size="lg"
        >
          {orderLoading ? (
